...
DeviceClient.prototype.callAction = function(serviceId, actionName, params, callback) {
  ...
  this.getServiceDescription(serviceId, function(err, desc) {
    ...
    console.log('call action %s on service %s with params %j', actionName, serviceId, params);
...
DeviceClient.prototype.subscribe = function(serviceId, listener) {
  ...
  this.getDeviceDescription(function(err, desc) {
    ...
    self.ensureEventingServer(function() {
      ...
      options.headers = {
        'HOST': options.host,
        'USER-AGENT': [OS_VERSION, 'UPnP/1.1', '0.1'].join(' '),
      ...
      var req = http.request(options, function(res) {
        ...
        function renew() {
          ...
          var req = http.request(options, function(res) {
            if(res.statusCode !== 200 && self.subscriptions[serviceId] !== undefined) {
	    ...
	    if (self.subscriptions[serviceId] === undefined) {
	       self.releaseEventingServer();
	       return; 
	    }

            self.subscriptions[serviceId].timer = setTimeout(renew, renewTimeout * 1000);
          });

          req.on('error', function(err) {

	    if (self.subscriptions[serviceId] !== undefined && self.subscriptions[serviceId].timer !== undefined) { 
                clearTimeout(self.subscriptions[serviceId].timer);
	    }
