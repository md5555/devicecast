...
function RaumfeldZone(device) {
    ...
    this.client.on('status', function(e) {
	    if(e.hasOwnProperty('CurrentTrackURI')) {

		logger.info("current URI: %s", e.CurrentTrackURI);

		if (self.castUri != null && e.CurrentTrackURI.localeCompare(self.castUri) != 0) {
		    self.emit('stopped', null);
		}	
	    }
    });
